...
def metrics_to_plot(history, params):
    ...
    json.dump(history_dict, open(f"h1units:{params.h1_units}_"
    ...
    for metric in metrics:
        axis[x, y].plot(epochs, history_dict[metric],
                        'bo', label=f'Training {metric}')
        axis[x, y].plot(
            epochs, history_dict[f"val_{metric}"], 'b', label=f'Validation {metric}')
    ...
    plt.savefig(f"h1units:{params.h1_units}_"
                f"h2units:{params.h2_units}_"
                f"l2reglambda:{params.l2_reg_lambda}_"
                f"lr:{params.learning_rate}_"
                f"batchsize:{params.batch_size}_"
