...
from model.utils import Params  # for the hyperparameters
from model.utils import set_logger  # for saving logs to know the flow
# used to train and evaluate a model
from model.training import train_and_evaluate
from model.input_fn import input_fn  # for the loading and preprocessing of data
# it is the file consisting of model's implementations
from model.model_fn import model_fn
# generation of visualizations and for plotting of the data
from model.visualize import metrics_to_plot

parser = argparse.ArgumentParser()
# as soon as i run the script (train.py) i might give more arguments so store these to know what i ask for.
...
parser.add_argument('--embeddings_dir', default='data/embeddings',
                    help="Directory containing pre-trained embeddings")
...
parser.add_argument('--sentences_length',
                    help="Max sentence length per author")
...
parser.add_argument('--word_embeddings',
                    help='Which pre-trained word embeddings to use, choices - are None and GloVe')
...
if __name__ == '__main__':
    ...
    # agar humne apne se koi parameter mai change krne ko bola then --> (script mai --h1units apni marzi se dala __so original params mai se vo particular jo parameter hai usko for that particular moment change kar denge)
    if args.h1_units:
    ...
    # Set the logger
    # ye log file bnane k liye
    set_logger(os.path.join(args.model_dir, 'train.log'))

    # data sets loading in variables
    ...
    # checking agar datasets exist krte
    ...
    # agar GloVe ko as an embedding use krre tab ye function chalao..
    if params.embeddings == "GloVe":
        ...
        embeddings_path = glove_dataset  # setting embedding path as Glove.

    # Create the input tensors from the datasets
    inputs = input_fn(pos_dataset, neg_dataset,
                      bert_dataset, params, embeddings_path)
    logging.info("- done.")
